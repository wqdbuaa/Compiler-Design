#include "D:\compiler\compiler\TargetCodeGenerator2.h"
#include <iostream>
#include <vector>
#include <string>
#include <fstream>
...
enum errsyntype{
	...
	bad_var_definition,//bad variable definition
	miss_ret_val,// int and char function has no return sentence;
	miss_ret_val_1,// has return sentence but no return value 
...
int para_count = 0;
int has_a_ret_statement = 0;/* has a return statement;
							if a int or char function has been detected, 
							ans this flag has been assigned to 1, all right
							else, wrong */
...
bool isCstDefinition(){
	...
	if (syn_sym == intcon ||syn_sym == charcon || syn_sym == comma){// if the current syn_sym is int
		...
		if (syn_sym == idsy){
			{
				int index = position_dup(watch_a, current_function, 1);
				if (index >= 0){
...
bool isMainFunction(){
	...
	if (syn_sym == voidsy){
		...
		if (syn_sym == mainsy){
			...
			syn_get_sym(1);
			{	
				int test[10] = {lparent, rparent,lbrace};
...
bool isExpression(){
	...
	clearSS();
	clearParts();
	if (syn_sym == minussy || syn_sym == plussy){
		string oprtt = (syn_sym == plussy || syn_sym == integer? "+" : "-");
		/*if (syn_sym != integer)*/
		syn_get_sym(1);
		if (isItem()){
			{
				itmd_code[ic].order = ic;
				itmd_code[ic].operand1 = "0";
				itmd_code[ic].op = oprtt;
				itmd_code[ic].operand2 = temp_var_cur;
				temp_var_cur = temp_var_gen();
				itmd_code[ic].result = temp_var_cur;
				ic++;
			}//intermediate code generating
			for (;syn_sym == plussy || syn_sym == minussy || syn_sym == integer;){
				string oprd1 = temp_var_cur;// may be a loose end
				string oprt= (syn_sym == plussy || syn_sym == integer? "+" : "-");
				if (syn_sym != integer)
					syn_get_sym(1);
				if (isItem()){
					{
...
bool isItem(){
	...
	clearSS();
	if (isFactor()){
		for (;syn_sym == starsy || syn_sym == dividesy;){
			string oprd1 = temp_var_cur;
...
bool isFactor(){
	...
	if (syn_sym == idsy && sym_buff[1] == lparent){
		...
		if (syn_sym == lbracket){
			...
			if (isExpression()){
				if (syn_sym == rbracket){
					...
					//addAssStcPart(watch_a);//assign sentence record added
					{
						int indexx = position(oprd1, current_function);
						tab[t - 1].typ = atab[tab[indexx].ref].eltyp;
					}
					syn_get_sym(1);
		...
		if (watch_a[0] == '+')
			temp_var_cur = watch_a.substr(1, watch_a.length());
		else
			temp_var_cur = watch_a;
		//temp_var_cur = num2str(num_buff[0]);
		syn_get_sym(1);
...
bool isStatement(){
	...
	else if (syn_sym == idsy && (sym_buff[1] == lparent || sym_buff[1] == asn ||sym_buff[1] == lbracket)){
		int t_i = position(watch_a);
		if (t_i < 0){
			int test[10] = {semicolon};
			error(undefined_identifier);
			testSkip(test, 1);
		...
		*/
		{
			...
			else if (tab[index].obj != variable && tab[index].obj != parameter){
				error(bad_asn);
				int test[10] = {semicolon};
				testSkip(test, 1);
				return true;
		...
		if (isExpression()){//preread has actually been done, ")" etc.
			...
			{	
				int test[10] = {semicolon};
				testToken(test, 1, bad_expression);
			}//handling error:
			return true;
		}
		...
		if (isExpression()){
			...
			{	
				int test[10] = {rbracket, semicolon};
				testToken(test, 2, bad_expression);
			}//handling error:
			return true;
		}
	}		
...
bool isIfStatement(){
	...
	else if (syn_sym == elsesy){
		{

			error(else_without_if);
			int test[10] = {rbrace, semicolon};
			testSkip(test, 2);
		}
		syn_get_sym(1);
		return true;
...
bool isCaseTable(string *cp, string *lo){
	...
	if (isCaseSubStatement(&case_para, &lab_out)){
		//syn_get_sym(1)//read the syn_sym next to ";",?????????????????? 
...
bool isRetFuncCallStatement(){
	...
	if (syn_sym == idsy && sym_buff[1] == lparent){	
		int parass = 0;
		{
			...
			parass = ftab[tab[index].ref].para_num;
		...
		temp_func_cur = watch_a;
		string func_cur = watch_a;
		syn_get_sym(1);
		if (syn_sym == lparent){
			syn_get_sym(1);
			if (syn_sym != rparent && isVluParaTable()){
				if (parass != para_count){
				...
				if (syn_sym == rparent){
					{
						temp_var_cur = temp_var_gen();
						itmd_add("call", func_cur/*temp_func_cur*/, "    ", temp_var_cur);//intermediate code generating
						int indexT = position(func_cur, func_cur);
						tab[t - 1].typ = tab[indexT].typ;
					}//intermediate code generating
					syn_get_sym(1);
	...
	if (syn_sym == idsy && sym_buff[1] == lparent){	
		...
		if (syn_sym == lparent){
			...
			if (syn_sym != rparent && isVluParaTable()){
				...
				if (syn_sym == rparent){
					{
						itmd_add("call", func_cur /*temp_func_cur*/, "    ", "    ");//intermediate code generating
					}//intermediate code generating
					syn_get_sym(1);
