...
int get_sym(){
    ...
    if (isLetter()){
		...
		if (resultValue == 0) {symbol = "idsy"; sym = idsy; changeUpToLow();}
    ...
    /*character*/
        ...
        while(ch == 32 || ch == 33 || (ch >= 35 && ch <= 126) /*|| (ch == '\"' && ch_last == '\\')*/){
            if (ch != 'n')
                catToken();
            if (isRevSlash()){
                id += '\\';
            ...
            if (ch == 'n'){
                if (id[id.length() - 1] == '\\'){
                    string tempp = id.substr(0, id.length() - 1);
                    id = tempp;
                }
                catToken();
            }
    ...
    else if (isNewLine()) {symbol = "newline"; sym = newline; id += ch;}
    /*comma, period, colon, semicolon, lparent, lbracket, lbrace, rparent, rbracket, rbrace,newline*/
    else if (isLss()){
        catToken();
        if (!get_char()) {
            return 0;
