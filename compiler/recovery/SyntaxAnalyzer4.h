			...
			if (isCondition()){//presume that will preread implicitly
				if (syn_sym == rparent){
					{	
						...
						itmd_add("jeq", temp_var_cur, "zero", jmp_lab_cur1);
					}//intermediate code generating
					syn_get_sym();
					if (isStatement()){//presume that will preread implicitly
						if (syn_sym == elsesy){
							...
							itmd_add("lab", "    ", "    ", jmp_lab_cur1);//intermediate code generating
							syn_symbol = "if statement has been detected.\n";
							outfile << syn_symbol;
							return true;
						}
...
bool isCondition(){
	...
	if (isExpression()){//preread implicit
		...
		if (syn_sym == leq || syn_sym == neq || syn_sym == eql ||
			...
			int ttt = syn_sym;
			string oprt = "";
			if (syn_sym == leq || syn_sym == gtr){
				oprt = "<=";
			}
			else if (syn_sym == lss || syn_sym == geq){
				oprt = "<";
			}
			else if (syn_sym == neq || syn_sym == eql){
				oprt = "==";
			}
			else{
				;
			...
			if(isExpression()){
				string oprd2 = temp_var_cur;
				{
					if (ttt == lss || ttt == leq || ttt == neq){
						temp_var_cur = temp_var_gen();
						itmd_add(oprt, oprd1, oprd2, temp_var_cur);
					}
					else{
						temp_var_cur = temp_var_gen();
						itmd_add(oprt, oprd2, oprd1, temp_var_cur);
					}
				}//intermediate code generating
