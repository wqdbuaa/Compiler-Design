			...
			if (syn_sym == asn){
				...
				syn_get_sym();
				{
					if (syn_typ_sym == intcon){
						int test[10] = {unsinteger, integer};
						testToken(test, 2, wrong_type);
					}
					else if (syn_typ_sym == charcon){
						int test[10] = {character};
						testToken(test, 1, wrong_type);
					}
				}//handling error: wrong type
				if (
...
bool isConst(){
	...
	clearSS();
	/*{	
		int test[10] = {integer, unsinteger, character};
		testToken(test, 3, not_a_const);
	}//handling error: find consts*/
...
bool isExpression(){
	...
	if (syn_sym == plussy ){
		...
		if (isItem()){
			...
			else if (syn_sym == minussy){// if it is a "-"
				string oprt= (syn_sym == plussy ? "+" : "-");
				//addAssStcPart(watch_a);//assign sentence record added
				syn_get_sym();
				if (isExpression()){
		...
		if (isItem()){
			...
			else if (syn_sym == minussy || syn_sym == integer){
				//string oprt= (syn_sym == plussy ? "+" : "-");
				//addAssStcPart(watch_a);//assign sentence record added
				//syn_get_sym();
				if (isExpression()){
...
bool isFactor(){
	...
	if (syn_sym == idsy && sym_buff[1] == lparent){
		...
		if (index < 0){
			error(miss_func);
			int test[10] = {idsy, integer, unsinteger, character, lparent};
			testSkip(test, 5);
		}
		else {
		...
		if (syn_sym == lbracket){
			...
			else if (tab[index].typ != arraycon){
				error(not_array);
				int test[10] = {idsy, integer, unsinteger, character, lparent};
				testSkip(test, 5);
			}//handling error:
...
bool isReadStatement(){
	...
	if (syn_sym == scanfsy){
		...
		else if (syn_sym == rparent || syn_sym == semicolon){
			error(bad_scf);
			return true;
		}
		else{
